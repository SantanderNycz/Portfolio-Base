...
export function PushSwapDemo() {
  ...
  const isSorted =
    stackA.length > 0 && stackB.length === 0 && stackA.every((val, i, arr) => i === 0 || arr[i - 1] <= val)

  return (
    <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
      <CardHeader>
        <CardTitle className="font-mono">push_swap Visualizer</CardTitle>
        <CardDescription>Sort numbers using two stacks and limited operations</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="min-h-[200px] p-4 bg-muted rounded-md border-2 border-primary/30">
              <div className="flex flex-col gap-2">
                ...
                {stackA.length === 0 && <div className="text-muted-foreground text-center text-sm">Empty</div>}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="min-h-[200px] p-4 bg-muted rounded-md border-2 border-accent/30">
              <div className="flex flex-col gap-2">
                ...
                {stackB.length === 0 && <div className="text-muted-foreground text-center text-sm">Empty</div>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-2">
            <Button onClick={sa} variant="outline" size="sm" className="font-mono bg-transparent">
              sa
            </Button>
            <Button onClick={sb} variant="outline" size="sm" className="font-mono bg-transparent">
              sb
            </Button>
            <Button onClick={pa} variant="outline" size="sm" className="font-mono bg-transparent">
              pa
            </Button>
            <Button onClick={pb} variant="outline" size="sm" className="font-mono bg-transparent">
              pb
            </Button>
            <Button onClick={ra} variant="outline" size="sm" className="font-mono bg-transparent">
              ra
            </Button>
            <Button onClick={rb} variant="outline" size="sm" className="font-mono bg-transparent">
              rb
            </Button>
            <Button onClick={rra} variant="outline" size="sm" className="font-mono bg-transparent">
              rra
            </Button>
            <Button onClick={rrb} variant="outline" size="sm" className="font-mono bg-transparent">
              rrb
            </Button>
          </div>
          <p className="text-xs text-muted-foreground">s=swap, p=push, r=rotate, rr=reverse rotate</p>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm">
            ...
            {isSorted && <span className="ml-4 text-accent font-bold">✓ Sorted!</span>}
          </div>
